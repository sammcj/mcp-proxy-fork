...
def load_named_server_configs_from_file(
    ...
    try:
        ...
        logger.error(f"Unexpected error opening or reading configuration file {config_file_path}: {e}")
    ...
    for name, server_config in config_data.get("mcpServers", {}).items():
        if not isinstance(server_config, dict):
            logger.warning(f"Skipping invalid server config for '{name}' in {config_file_path}. Entry is not a dictionary.")
            continue
        ...
        if not command:
            logger.warning(f"Named server '{name}' from config is missing 'command'. Skipping.")
            continue
        ...
        logger.info(f"Configured named server '{name}' from config: {command} {' '.join(command_args)}")
